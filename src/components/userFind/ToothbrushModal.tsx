...
const ToothbrushModal: React.FC<Props> = ({
  isOpen = false,
  onClose,
  autoShow = true,
  name = '테스트',
  ...
  // ✅ 안전한 타임아웃(ms) — config 로딩 전엔 5000ms
  const timeoutMs = useMemo(
    () => Number(config?.toothbrushModalTimeout ?? 5000),
    [config?.toothbrushModalTimeout]
  );
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && showModal) handleClose();
    };
    document.addEventListener('keydown', handleKeyDown);
    return () => document.removeEventListener('keydown', handleKeyDown);
  }, [showModal]);

  // ✅ 일정 시간 후 홈으로 이동 (런타임 config 사용)
  useEffect(() => {
    if (!showModal) return;
  ...
  const backgroundOverlayStyle: React.CSSProperties = {
    ...
    background: 'rgba(49, 49, 49, 0.6)', zIndex: 1
  };

  const modalStyle: React.CSSProperties = {
    position: 'relative', width: '932px', height: '675px', background: '#FFFFFF',
    borderRadius: '41px', boxShadow: '2px 2px 2px 0px rgba(42, 73, 148, 0.09)',
    zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center',
    animation: 'modalFadeIn 0.3s ease-out', padding: 0
  };

  const modalContentStyle: React.CSSProperties = {
    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px',
    width: '680px', paddingLeft: '126px', paddingRight: '126px', marginTop: '80px'
  };

  const modalTitleStyle: React.CSSProperties = {
    fontFamily: 'Pretendard, -apple-system, BlinkMacSystemFont, system-ui, sans-serif',
    fontWeight: 700, fontSize: '72px', lineHeight: '1.4', letterSpacing: '-0.025em',
    textAlign: 'center', color: '#004F99', margin: 0, height: '90px',
    display: 'flex', alignItems: 'center', justifyContent: 'center'
  };

  const modalSubtitleStyle: React.CSSProperties = {
    fontFamily: 'Pretendard, -apple-system, BlinkMacSystemFont, system-ui, sans-serif',
    fontWeight: 700, fontSize: '68px', lineHeight: '1.4', letterSpacing: '-0.025em',
    textAlign: 'center', color: '#F9B657', margin: 0, height: '90px',
    display: 'flex', alignItems: 'center', justifyContent: 'center'
  };
