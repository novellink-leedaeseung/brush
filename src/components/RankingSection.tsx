        ...
        {topThree.map(user => {
          ...
          return (
            <React.Fragment key={user.rank}>
              {/* 프로필 */}
              <img style={{ ...rankStyle, position: 'absolute', borderRadius: '999px',
                border: `3px ${user.borderColor} solid` }} src={user.profileImage} alt={user.name}/>
              {/* 랭킹 배지 */}
              <div style={badgeStyle}>{user.rank}</div>
              {/* 이름 */}
              <div style={{ ...namePos, position: 'absolute', fontSize: '36px', fontWeight: 600 }}>
              ...
              {/* 식사 태그 */}
              <div style={{ ...mealPos, position: 'absolute', background: '#B2D7FF', borderRadius: 8 }}>
                <div style={{ color: '#227EFF', fontSize: '16px', fontWeight: 600 }}>
