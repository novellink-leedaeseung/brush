...
const hh = String(today.getHours()).padStart(2,'0'); const mm = String(today.getMinutes()).padStart(2,'0'); const ss = String(today.getSeconds()).padStart(2,'0')
...
const response = await fetch('/api/save-photo', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ imageData: imageDataUrl, fileName }) })
const result = await response.json()
if (response.ok && result.success) return { success: true, message: result.message, fileName: result.fileName, filePath: result.filePath, timestamp: result.timestamp }
